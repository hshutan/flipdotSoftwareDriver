...
void mcp::UpdateSign()
{
  ...
  // Print static start of image code
...
void mcp::ConvertBitmapToBytestream()
{
  ...
  for (int x = 0; x < xSize; x++) { // Loop thru each column
    ...
    for (int y2 = 15; y2 > 7; y2--) { // Loop thru the second 8 bits of a column
      ...
      if (y2 > 8) { // This if prevents the bitshift << from happening more than 8 times
...
void mcp::CloseSign()
{
  // This can be run before power is shutdown to the sign.
...
void mcp::PrintString(String in)
{
  ...
  delay(10); // Delay after each line is sent
